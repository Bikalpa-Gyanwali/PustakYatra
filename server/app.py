...
def recommend():
    ...
    try:
        ...
        data = []
        # We will skip the first result (best match) if it has a similarity score of 100%
        for i in similar_items:
            # Skip the book with 100% similarity score (same book entered by user)
            if pt.index[i[0]] == best_match and i[1] == 1.0:
                continue
        ...
        # Preparing best match details to send along with recommendations
        best_match_data = {
            'title': best_match,
            'author': books[books['Book-Title'] == best_match]['Book-Author'].values[0],
            'image': books[books['Book-Title'] == best_match]['Image-URL-M'].values[0],
            'score': score
        }

        return jsonify({'best_match': best_match_data, 'recommendations': data})
